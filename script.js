...
window.onload = function () {
  if (getCookie("server")) {
    window.location = `https://blackcatbot.tk/app/?server=${getCookie("server")}`;
    deleteCookie("server");
  }
};
...
document.getElementById("query").onclick = function () {
  if (document.getElementById("serverid").value.length !== 18 || isNaN(document.getElementById("serverid").value)) {
    ...
    window.location.href = "https://blackcatbot.tk/app/?server=" + document.getElementById("serverid").value;
  }
};
